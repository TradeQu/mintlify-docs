...
In AML and compliance workflows, different actors are responsible for different steps. Include an `actor` object on each `POST /v1/decisions/record` call to record who performed each step:
...
Each `actor.id` is cryptographically bound to its step's integrity hash. Auditors querying the chain can see exactly who acted at each step.

See the [Actor Attribution guide](../guides/actor-attribution) for schema details and trust model considerations.
