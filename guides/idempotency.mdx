...
<Info>
  **Evaluate vs Record** — Idempotency applies to [`record`](concepts/overview#evaluate-vs-record), the endpoint that persists a decision. `evaluate` records no decision, so it has no decision to deduplicate. It is not a no-op write, though: resolving the active policies may append one row to the tenant's append-only `policy_snapshots` table, returned as `policy_snapshot_id`. That append is content-addressed — a repeat call resolving to the same policies reuses the existing snapshot and appends nothing.
</Info>
...
| Key already used, **same content** | `200 OK` | `false` | The original decision is returned immediately — no re-evaluation. |
| Key already used, **different content** | `409 Conflict` | — | Refused. Nothing is recorded, and the stored decision is **not** returned. |

The second case returns the stored decision without running policies again, so retries are both safe and cheap.

<Warning>
  **A key is bound permanently to the first request recorded under it.** Sending a *different* request under a key that is already in use is refused with `409` and the error code `IDEMPOTENCY_KEY_REUSED` — MeshQu will not hand you a receipt for content you did not submit, and will not silently record your request under someone else's key.

  The `409` body tells you which of the two things to do:

  ```json
  {
    "error": {
      "code": "IDEMPOTENCY_KEY_REUSED",
      "message": "This idempotency_key is already bound to a different request for this tenant. The submitted request was NOT recorded, and the previously recorded decision was NOT returned.",
      "details": {
        "existing_decision_id": "…",
        "existing_recorded_at": "…",
        "submitted_request_fingerprint": "…",
        "recorded_request_fingerprint": "…",
        "fingerprint_covers": ["context.decision_type", "context.fields", "…"],
        "fingerprint_excludes": ["correlation_id", "…"],
        "fingerprint_ignores": ["evidence_manifest", "evidence_manifest_digest", "action", "options.chain"],
        "remedy": {
          "to_record_this_request": "Resubmit with a NEW idempotency_key. …",
          "to_replay_the_recorded_decision": "Resubmit the ORIGINAL request body unchanged under this key, …"
        }
      }
    }
  }
...
```
First request:  POST /record { idempotency_key: "payment-12345", …context A } → 201, is_new: true
Retry request:  POST /record { idempotency_key: "payment-12345", …context A } → 200, is_new: false
Reused key:     POST /record { idempotency_key: "payment-12345", …context B } → 409, IDEMPOTENCY_KEY_REUSED
```
