...
Use **`record`** for settlement decisions — it evaluates *and* persists a signed receipt with an idempotency key, which is exactly what a compliance-critical path needs. (Use [`evaluate`](/concepts/overview#evaluate-vs-record), which records no decision, only for dry-run testing or non-settling previews.)
