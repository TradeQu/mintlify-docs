...
For the hot path — gating thousands of routine tool calls — use **`evaluate`**. It records no decision, so it is cheap to call on every action and leaves no decision to page through later. It is not a pure read: resolving the active policies may append one row to the tenant's append-only `policy_snapshots` table, returned as `policy_snapshot_id`. On a hot path that append is rare — a repeat call resolving to the same policies reuses the existing snapshot and appends nothing, so a steady policy set costs one row, not one per call. Persist only the consequential decisions with `record` (next section).
...
- [Core Concepts — Evaluate vs Record](/concepts/overview#evaluate-vs-record) — when to use the dry-run gate vs the recording one.
