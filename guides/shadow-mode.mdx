...
If you are replacing or supplementing a system that already makes the decision, you can run MeshQu *alongside* it before MeshQu becomes authoritative. This is a client-side pattern — it doesn't require any MeshQu feature beyond the `evaluate` endpoint:
...
<Note>
  Use `evaluate` (not `record`) for throwaway comparisons — it records no decision, so the comparison leaves no decision behind. It does not leave the tenant untouched, though: resolving the active policies may append one row to the append-only `policy_snapshots` table, returned as `policy_snapshot_id`; a repeat call resolving to the same policies appends nothing. Switch to `record` once you want a durable, replayable audit trail of each decision. See [Evaluate vs Record](concepts/overview#evaluate-vs-record).
</Note>
