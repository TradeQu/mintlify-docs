...
<Warning>
**Trust roots must come from a channel MeshQu does not control.** The verifier authenticates signatures against keys it pins out-of-band — never against any key list carried inside the receipt or bundle. If a verifier ever authenticated a receipt using keys the receipt itself supplied, an operator could self-sign anything and have it "pass." Pin the public key fingerprints through a channel your policy accepts and treat that pinned copy as the root. Do **not** use `GET /v1/.well-known/signing-keys` as a trust root: it is deprecated and self-asserted — the producer vouching for itself — so authenticating against it proves only that the producer signed what the producer published.
</Warning>
...
3. Paste the modified receipt back in. **Integrity → Tampered.** The recomputed hash no longer matches the stored `integrity_hash`, because you changed a field the hash covers. **Signature → still Valid** — and that is correct behaviour, not a gap. The signature was made over the *stored* integrity hash, which you have not touched; it proves MeshQu attested to that hash. Binding the hash to the content is the integrity check's job.
4. Now try to paper over the edit: rewrite the stored `integrity_hash` so the integrity check passes again. Paste it back. **Signature → Invalid.** The signature covers the hash you just rewrote, and you cannot forge a new valid signature without MeshQu's private key.

<Note>
**Why the pairing matters.** Neither check alone is tamper-evidence: a content edit leaves the signature intact, and a hash rewrite leaves the content self-consistent. Together they close the loop — no single-field alteration survives both checks, and making a tampered receipt pass *both* would require MeshQu's signing key. That is exactly the property a regulator wants: tamper-evidence that cannot be papered over.
</Note>
...
The [Trust Model](/security/trust-model) lays out the assurance ladder (L1–L5) that progressively reduces these dependencies. The top rung — independently governed trust — is a property of how a deployment distributes and governs its trust roots, not a receipt feature still being built.

## See also

- [Verification Bundle](/concepts/verification-bundle) — the ten sub-claims and the offline archive format.
- [Receipt Reference](/concepts/receipt-reference) — exactly which fields are bound into the hash and signature.
- [Trust Model](/security/trust-model) — the L1–L5 assurance ladder and what each rung proves.
