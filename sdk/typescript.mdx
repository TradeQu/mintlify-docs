...
description: "Early-access convenience wrapper over the REST API: client setup, decisions, policies, and alerts"
---

<Warning>
  The `@meshqu/client` TypeScript client is **early access**. It is a convenience wrapper over the [REST API](../api/reference), which is the primary, supported integration surface. The client is **not published to npm**, and its public surface — method signatures, options, and exported types — is **not stable and may change without notice in any release**. Pin a known-good version, and prefer the REST contract for anything you depend on. Contact [contact@meshqu.com](mailto:contact@meshqu.com) for access.
</Warning>
...
// result.result.dry_run: true (always, on this call)
// result.result.integrity_hash: string (SHA-256; reproducible, not signed)
```

`client.dryRun()` is an alias for `client.evaluate()`.

<Note>
A dry-run result carries **no** `signature`, `signature_kid` or `signature_algorithm` — the fields are absent from the response, not null, and the client's `EvaluateResponse` type does not declare them. Do not branch on them here. Use `record` when you need a signed receipt.
</Note>
...
// A `when` gate compares strictly by default — 'Equity' would NOT match the
// gate above. To accept any casing of a string value, opt in per gate:
//   when: { field: 'instrument_type', equals: 'equity', case_sensitive: false }
// Folding is string-vs-string only (booleans and numbers always compare
// strictly), and omitting the key keeps the strict behaviour every existing
// rule already relies on. Available from API release 1.10.0.

// Get
const fetched = await client.getPolicy(policy.id);

// Update metadata
await client.updatePolicy(policy.id, { shadow_mode: true });
...
<Info>
  The typed exception classes below are part of the early-access client surface and may change without notice. The authoritative contract is the HTTP status and error code returned by the REST API — see [Errors & Status Codes](../api/errors). Match on those when you need stability.
</Info>
