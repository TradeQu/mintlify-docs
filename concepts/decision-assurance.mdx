...
- DELETE on `meshqu.decisions` or `meshqu.policy_snapshots` is blocked
  at the trigger level.
- UPDATE on integrity-bound columns is blocked. The only legitimate
  post-INSERT mutation is `result.transparency_anchor` (the async-Rekor
  patch path).
