...
Rules can include an optional `when` clause for conditional applicability. A rule with `when` is only evaluated when the clause matches the decision context — otherwise it returns NA (not applicable) and does not affect the outcome. NA rules still appear on the decision receipt with a human-readable reason recording that the rule was considered and why it didn't apply. That record is stored on the receipt but is not itself bound into the integrity hash — see [What Is and Isn't Bound](/concepts/receipt-reference) (§14).
...
| --- | --- | --- |
| **Decision recorded** | No — no decision is stored, no decision id is returned, and nothing is retrievable as a decision afterwards | Yes — context, verdict, and metadata stored |
| **Result signed** | No — the result carries `dry_run: true` and no signature fields at all. `integrity_hash` is reproducible, not attested | Yes — a signed Decision Receipt |
| **Other writes** | Resolving the active policies may append one row to the tenant's append-only `policy_snapshots` table, whose id is returned as `policy_snapshot_id`. When an existing snapshot matches, nothing is appended | Same snapshot resolution, plus the decision itself |
