...
- **Not a rules engine you embed.** MeshQu runs as a standalone service. Your application calls it over HTTP.
- **Not an execution layer.** MeshQu returns a verdict — it never blocks, allows, or modifies operations on its own.
- **Not a permissions system.** It does not manage who can log in or what resources they can access. Use it alongside your existing IAM.
- **Not a real-time data feed.** It evaluates the context you send. It does not subscribe to market data or external sources.
- **Not an explainability tool.** MeshQu proves that a decision followed policy, not why a model produced a score.
