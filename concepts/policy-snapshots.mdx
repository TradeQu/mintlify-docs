...
- **At the database** — an immutability trigger rejects every `UPDATE` and `DELETE` on the snapshot table. This binds the API's runtime role and every application caller; it does **not** bind the table owner or a DB superuser, who can disable any trigger. See [Storage-layer immutability](/concepts/decision-assurance#storage-layer-immutability-what-it-enforces-and-against-whom) for what that bound does and does not leave standing.
