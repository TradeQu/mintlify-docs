...
Returns a signed decision receipt by ID. The receipt is enough to verify integrity and signature offline given the corresponding public signing key (see [JWKS](#jwks)).
...
Returns a full Verification Bundle — receipt + policy snapshot + chain proofs + transparency proof + trust roots, under a digest-of-digests manifest. See [Verification Bundle](../concepts/verification-bundle) for the on-disk layout and sub-claim taxonomy.
