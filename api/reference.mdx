...
**Determinism guarantee:** Given the same decision context and the same policy snapshot, MeshQu will always return the same verdict. The `policy_snapshot_hash` and `integrity_hash` in each response let you verify this independently. Determinism applies strictly to MeshQu's policy evaluation logic. It does not extend to the contents of `fields`, `metadata`, or `evidence` — those are supplied by your application and outside MeshQu's control.
...
Evaluations are designed for the hot path. Typical response times are single-digit milliseconds for policy evaluation, excluding network latency.
