...
**Determinism guarantee:** Given the same decision context and the same policy snapshot, MeshQu will always return the same verdict. The `policy_snapshot_hash` and `integrity_hash` in each response let you verify this independently.
...
Validates policy rules without creating or modifying any policy.
