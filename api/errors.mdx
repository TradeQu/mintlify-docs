...
| `SNAPSHOT_NOT_FOUND` | Decision replay; `evaluate` with `options.snapshot_id` | The policy snapshot could not be located. Returned as `404`. |
| `POLICY_VERSION_NOT_RATIFIED` | `evaluate` with `options.snapshot_id` | The named snapshot pins a policy version that is not the active ratified one — draft, submitted or superseded. Returned as `400`. |
| `POLICY_VERSION_RATIFICATION_UNKNOWN` | `evaluate` with `options.snapshot_id` | The named snapshot's ratification status could not be established, or it names no policy versions. Returned as `400`. |
